...
export async function addOrUpdateUser(
  ...
  const update = {
    ...
    // Upsert option ensures that the document is created if it dosen't exist
    // The new: true option in the options object ensure that the method returns the udated documnet after the operation is complete. if we don't set new: true, the method will return the original documnet before the upadte.

    // In summary, the code will either update an exsting user's document with new translations or create a new user document with the given userId, and translations, and save it into the database.
